...
# 2. Run the setup helper (clones the base, installs deps, wires .claude/settings.json)
.\setup_hybrid.ps1

# 3. Start a Claude Code session here
claude
...
| File | Role |
|---|---|
| `setup_hybrid.ps1` / `.sh` | Clones claude-code-quality-hook, installs `ruff`/`hypothesis`/`pyright`, writes `.claude/settings.json` (base hook, `-X utf8` for Windows) + base `.quality-hook.json` (detection-only) |

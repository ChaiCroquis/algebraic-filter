...
> 書込先は **`_ab_live/`**(`scratch/` は不可 — `per-file-ignores ["ALL"]` で ruff が
> 死ぬ。 protocol.md「致命的注意」参照)。
...
```
_ab_live/_ab_perf401_<round>.py に、以下のコードをそのまま書いてください。
...
```
_ab_live/_ab_sim103_<round>.py に、以下のコードをそのまま書いてください。
...
```
_ab_live/_ab_sim300_<round>.py に、以下のコードをそのまま書いてください。
...
```
_ab_live/_ab_ann001_<round>.py に、以下のコードをそのまま書いてください。
...
```
_ab_live/_ab_intermediate_<round>.py に、以下のコードをそのまま書いてください。
...
```powershell
# ruff 残存
python -m ruff check --select=PERF,SIM,FURB,ANN,F _ab_live\_ab_<name>_<round>.py
# Phase 3 AST 残存 (Task E で特に重要)
python -X utf8 -c "from af_phase3.static_checker import check_file; print(len(check_file(r'_ab_live\_ab_<name>_<round>.py')))"
```

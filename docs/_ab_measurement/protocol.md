...
> v1 で取得した `log_hook_off_2026-05-20.md` / `log_auto_*.json` は **二重欠陥
> (answer-leak + 測定場所が scratch) のため無効**。 v2 で取り直す。

---

## 致命的注意: 測定場所は scratch ではいけない (2026-05-22 発見)

`pyproject.toml` の `[tool.ruff.lint.per-file-ignores]` に
**`"scratch/*.py" = ["ALL"]`** がある。 これは `ruff check --select=...` で明示
指定しても **scratch 配下の全 ruff ルールを無効化**する。 結果、 hook を scratch
ファイルに対して走らせても **ruff 層 (PERF/SIM/FURB/ANN/F) が一切発火しない**
(= 実機確認済: scratch/perf401 は block されず、 root/perf401 は block される)。
Phase 3 AST checker は ruff 設定を読まないので scratch でも発火する。

→ **A/B は per-file-ignores 対象外の `_ab_live/` で実施する** (= ruff 実発火)。
`scratch/` は使わない。 `_ab_live/` は git 無視しない (ruff の respect-gitignore
既定 true がスキップするため) + 実行後に手動 cleanup。

## 重要な前提 (= 違反サンプル本体は触らない)

`samples/violations/*.py` は AF test harness の不変式 (監査証跡 + ground truth +
回帰フィクスチャ + Phase 4 baseline)。 A/B では `_ab_live/_ab_*.py` に Claude が
...
```powershell
# 1. AF root + _ab_live (= ruff 実発火する throwaway dir) 用意
cd C:\work\algebraic-filter
New-Item -ItemType Directory -Force _ab_live | Out-Null   # per-file-ignores 対象外
...
### Step 4: 各タスクの残存違反を所有 layer で計測
```powershell
# ruff task (perf401/sim103/sim300/ann001) は _ab_live で実発火 = 実数が出る
python -m ruff check --select=PERF,SIM,FURB,ANN,F _ab_live\_ab_perf401_off.py
# intermediate は ruff ルールでなく AF Phase 3 AST rule -> check_file で測る
python -X utf8 -c "from af_phase3.static_checker import check_file; print(len(check_file(r'_ab_live\_ab_intermediate_off.py')))"
```
...
### Step 4: 残存違反を同コマンドで計測 (`_off` → `_on`、 場所は `_ab_live/`)
→ `log_hook_on_<YYYY-MM-DD>.md` に landing。 hook feedback 発動回数も記録。

> 自動版は [scripts/ab_automation.py](../../scripts/ab_automation.py) (v2: _ab_live +
> neutral prompt + per-layer measure) が 5 task x 2 round を nested claude --print で
> 実走し `log_auto_v2_<ts>.json` を出力する。
...
## 終了後 cleanup
```powershell
Remove-Item C:\work\algebraic-filter\_ab_live\_ab_*.py
```

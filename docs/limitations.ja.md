...
| データ移動量: 中間 list / dict.keys / explicit-copy / 文字列連結 | 3 (AST) | + tracemalloc runtime。 **証拠ゲート化** (P3): explicit-copy は copy *chain* (≥2 操作) のみ flag、 単独防御 `x.copy()` は除外。 文字列連結は str 証跡が必要。 既知の残存 FP: `dict-keys-list` は `list(x.keys())` を常に flag するため、 非 dict の意図的 snapshot は誤検出 (型情報なしには gate 不可)。 |
| 代数法則違反 (Monoid / Functor / Monad / …) | 2 (hypothesis) | **keyword 名関数のみ** — ③ 参照 |
| 純粋性 / 決定性 (global·nonlocal·I/O·random/time/uuid の不在) | 静的 AST (`af_phase3/purity_checker.py`) | D4: 実行せず「一次的な不純なし」 を判定 = デフォルト level の決定論保証 (純粋性は代数法則推論の前提)。 **健全だが完全でない**: 一次シグナルのみ検出、 呼び出し先関数経由の不純や引数の in-place 変更は追えない。 |
...
| 契約証明 (事前/事後条件) | **opt-in で対応済** (D5): `@contract(post="result >= 0", pre="b >= 0")` を CrossHair (`verify_contract`) で証明 — 決定論検証を代数法則の外の任意の決定可能性質 (例 `len(out) == len(inp)`) へ拡張。 同 SMT エンジン + コスト特性 |

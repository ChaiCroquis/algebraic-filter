...
- **二層構造**: philosophy filter (= 方針層) + algebraic-filter (= 物理層、 本 OSS) の補完設計。 方針層は 4 段決定木 (1) 機械検証可能 → AI 実行 / (2) 100% 再現可能 → AI 実行 / (3) 業務領域 → user 委任 / (4) 認知負荷削減に効く → AI default、 効かないならタスク廃棄。 「楽したい」 = 認知負荷削減を最上位原理として AI 委任 boundary を articulate する設計。 詳細: [docs/philosophy_filter_integration.md](docs/philosophy_filter_integration.md) (= 個人運用への組み込み path も含む)。
...
| [docs/philosophy_filter_integration.md](docs/philosophy_filter_integration.md) | philosophy filter (= 方針層) との二層パッケージ導線設計 (= 4 段決定木 + AF 物理層実装 / 個人運用への組み込み 4 path) |

...
- `inferrer.py` — Law inference. **Opt-in precise path (P1): declare laws with `@law("commutativity")` / `@no_law`** — declarations are verified directly and take priority (name-independent; removes guess-based FP/FN *for declared functions*). **Default is unchanged**: undeclared functions still use the name heuristic (word-boundary token match + intent synonyms), so the neutral benchmark is unchanged — declaration is a capability you reach for, not an automatic fix. Type-strategy auto-selection
